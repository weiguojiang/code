...
#define DP_VPORT_HASH_BUCKETS	256
...
//#endif
    ...
    cvmx_rwlock_wp_lock_t lock; // for ports list
    int dp_ifindex;
    char dp_name[VPORT_NAME_LENGTH_MAX + 1];
};
...
extern int ovs_flow_cmd_del(struct sw_flow_key *key, int dp_ifindex);
extern int ovs_vport_cmd_del(char * port_name_ptr,
        int dp_ifindex, u32 port_no);
