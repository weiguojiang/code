...
//	struct pcpu_sw_netstats __percpu *percpu_stats;
   struct ovs_pcpu_sw_netstats percpu_stats;
...
struct vport_ops {
	...
	void (*destroy)(struct vport *, int type);
...
{
	//return (struct vport *)((u8 *)priv /* - ALIGN(sizeof(struct vport), VPORT_ALIGN) TODO just for compile OK , ALIGN issue*/);

	return (struct vport *)((u8 *)priv - ((((sizeof(struct vport))) + ((typeof(sizeof(struct vport))) (8) - 1)) & ~((typeof(sizeof(struct vport))) (8) - 1)));
				      ...
				      const void *start, unsigned int len)
{
	/*if (skb->ip_summed == CHECKSUM_COMPLETE)
		skb->csum = csum_add(skb->csum, csum_partial(start, len, 0)); TODO issue */
}
#endif /* vport.h */
